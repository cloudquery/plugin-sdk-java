...
public class GlobTest {
  ...
  public void testEmptySubject() {
    for (String s : List.of("", "*", "**", "***", "****************", GLOB.repeat(1000000))) {
      assertGlobMatch(s, "");
    }
  ...
  public void assertGlobMatch(String pattern, String subject) {
    assertTrue(
        Glob.match(pattern, subject),
        String.format("\"%s\" should match \"%s\"", pattern, subject));
  }

  public void assertNotGlobMatch(String pattern, String subject) {
    assertFalse(
        Glob.match(pattern, subject),
        String.format("\"%s\" should not match \"%s\"", pattern, subject));
  }
}
