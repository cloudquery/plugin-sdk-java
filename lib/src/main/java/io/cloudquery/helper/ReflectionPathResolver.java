...
public class ReflectionPathResolver {
   ...
   * </pre>
   *
   * <p>Then the following are valid paths to retrieve the associated values:
   *
   * <p>`name` `child.name` NOTE: this implementation is currently very simplistic and only supports
   * simple field and nested field resolution. It does not support collection resolution - unlike
   * the Go SDK which uses <a href="https://github.com/thoas/go-funk#get">go-funk</a>.
   *
